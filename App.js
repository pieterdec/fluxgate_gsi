...
function setupRecordingControls() {
    ...
    function render(status) {
        ...
        if (status.recording) {
            ...
            statusEl.textContent = `Stopped (${status.saved ? 'saved' : 'not saved'}): ${status.file}`;
            statusEl.classList.remove('is-recording');
        } else {
    ...
    async function refreshStatus() {
        ...
        isRecording = !!status.recording; // resume the correct point color if the page was reloaded mid-recording
        render(status);
    }

    // Start only needs a name when it's about to create a brand-new file
    // (no active session yet - either a fresh boot or right after Clear).
    // If a session is just paused (Stop was pressed, current_file still
    // set), pressing Start silently resumes it.
    startBtn.addEventListener('click', async () => {
        const status = await fetchStatus();
        if (status.file) {
            const s = await postRecord('start');
            isRecording = true;
            render(s);
        } else {
            openStartNameModal();
    ...
    clearBtn.addEventListener('click', async () => {
        // Warn before losing easy access to data that hasn't been saved recently.
        const status = await fetchStatus();
        const hasActiveData = !!status.file;
        const savedRecently = status.saved_at && (Date.now() - status.saved_at) < 60000;
        if (hasActiveData && !savedRecently) {
        ...
        // Clear resets all the way back to the "just booted" state: no
        // active file, not recording. It does NOT start a new recording -
        // you still need to press Start for that (which will then prompt
        // for a name, same as a true first start).
